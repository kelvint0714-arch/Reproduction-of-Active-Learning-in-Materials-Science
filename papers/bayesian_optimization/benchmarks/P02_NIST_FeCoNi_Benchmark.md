...
论文将主动学习作为序贯实验设计的上位概念，但实际基准任务是寻找目标性质最优值，并以 regret 和命中速度评价。因此，本仓库按主要任务将它归为**贝叶斯优化（BO）基准**，而不是模型学习型主动学习。
...
这篇论文是材料 BO 采集函数基准主线。未来将 GP 换成 RF、PBNN 或 GNN 时，应尽量保持这里的候选池、预算与评价协议，才能判断提升来自模型还是选样策略。
