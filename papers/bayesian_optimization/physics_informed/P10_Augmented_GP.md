...
论文的方法可以服务两类序贯任务，但其主要示例与论述集中在优化/探索以及结构化 GP-BO。本仓库因此将它归入**物理先验增强的贝叶斯优化方法**，同时保留其可用于主动学习的次级标签；它不是经典的模型学习型主动学习论文。
...
价值非常高。它是“小样本 + 物理知识 + 贝叶斯优化/主动选样”的直接路线之一，通常比在缺少明确 PDE 时硬套 PINN 更现实；但不应把它当作经典模型学习型主动学习的替代。
